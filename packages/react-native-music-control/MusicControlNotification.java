...
public class MusicControlNotification {
    ...
    // Flyme状态栏歌词显示标志
    private int mFlagAlwaysShowTicker = 0;
    private int mFlagOnlyUpdateTicker = 0;
    private boolean isFlymeSupported = false;
    ...
    // 获取Flyme状态栏歌词显示标志
    public int getFlagAlwaysShowTicker() {
        return mFlagAlwaysShowTicker;
    }

    // 是否支持Flyme状态栏歌词
    public boolean isFlymeSupported() {
        return isFlymeSupported;
    }

    // Helper方法：使用反射获取字段值
    private Object getFieldStepwise(Class<?> desClass, Object desObj, String fieldName) throws Exception {
        try {
    ...
    // 检查Flyme状态栏歌词支持
    private void checkFlymeSupport() {
        try {
            ...
            mFlagAlwaysShowTicker = 0;
            mFlagOnlyUpdateTicker = 0;
        }
    }
    ...
    public synchronized Notification prepareNotification(NotificationCompat.Builder builder, boolean isPlaying) {
        ...
        if (isFlymeSupported) {
               if(Build.VERSION.SDK_INT >= Build.VERSION_CODES.KITKAT) {
                ...
                mNotification.flags |= mFlagAlwaysShowTicker;
                mNotification.flags |= mFlagOnlyUpdateTicker;
        } else {
            mNotification.flags &= ~mFlagAlwaysShowTicker;
            mNotification.flags &= ~mFlagOnlyUpdateTicker;
        }
